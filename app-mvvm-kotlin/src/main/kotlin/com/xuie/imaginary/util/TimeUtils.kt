...
class TimeUtils {
  ...
  companion object {
    ...
    fun getStringByFormat(date: Date, format: String): String? {
      val mSimpleDateFormat = SimpleDateFormat(format, Locale.CHINA)
      var strDate: String? = null
      try {
    ...
    fun getStringByFormat(strDate: String, format: String): String? {
      ...
      try {
        ...
        val mSimpleDateFormat = SimpleDateFormat(format, Locale.CHINA)
        c.time = mSimpleDateFormat.parse(strDate)
        val mSimpleDateFormat2 = SimpleDateFormat(format, Locale.CHINA)
    ...
    val currentDay2: String?
      get() {
        var curDateTime: String? = null
        try {
          val mSimpleDateFormat = SimpleDateFormat(dateFormatYMDHMS, Locale.CHINA)
          val c = GregorianCalendar()
          c.add(Calendar.DAY_OF_MONTH, 0)
          curDateTime = mSimpleDateFormat.format(c.time)
        } catch (e: Exception) {
    ...
    fun formatDateStr2Desc(strDate: String, outFormat: String): String? {
      ...
      val df = SimpleDateFormat(dateFormatYMDHM, Locale.CHINA)
      ...
      try {
        ...
        if (d == 0) {
          ...
          when {
            h > 0 -> return h.toString() + "小时前"
            h < 0 -> return Math.abs(h).toString() + "小时后"
            h == 0 -> {
    ...
    fun getWeekNumber(strDate: String, inFormat: String): String {
      ...
      val df = SimpleDateFormat(inFormat, Locale.CHINA)
      try {
    ...
    fun getChatTimeForShow(time: Long): String? {
      return if (TimeUtils.isToday(time)) {
        TimeUtils.getStringByFormat(time, TimeUtils.dateFormatHMofChinese)
      } else {
        TimeUtils.getStringByFormat(time, TimeUtils.dateFormatMDHMofChinese)
      }
    }
    ...
    fun getDatelongMills(fomat: String, dateStr: String): Long {
      val sdf = SimpleDateFormat(fomat, Locale.CHINA)
      var date: Date? = null
      try {
        date = sdf.parse(dateStr)
      } catch (e: ParseException) {
