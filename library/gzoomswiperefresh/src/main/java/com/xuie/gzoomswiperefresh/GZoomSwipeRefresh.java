package com.xuie.gzoomswiperefresh;

/*
 * Created by gzoom on 2017/2/4.
 * https://github.com/SGZoom/DailyWidget
 */
/*
 ...
 * </p>
 */
public class GZoomSwipeRefresh extends ViewGroup implements NestedScrollingParent,
    ...
    private static final String LOG_TAG = GZoomSwipeRefresh.class.getSimpleName();
            ...
            if (mRefreshingBottom) {
                ...
                if (mNotifyBottom) {
                    if (mListenerBottom != null) {
                        ...
                        if (DEBUG) Log.e("fish", "mListenerBottom!= null");
                    } else {
                        if (DEBUG) Log.e("fish", "mListenerBottom == null");
                    }
                }
     ...
     * @param context
     */
    public GZoomSwipeRefresh(Context context) {
        this(context, null);
    }
     ...
     * @param attrs
     */
    public GZoomSwipeRefresh(Context context, AttributeSet attrs) {
        super(context, attrs);
        ...
        if (DEBUG) Log.e("fish", "init+-+windowHeight==" + windowHeight);
        mOriginalOffsetBottom = windowHeight - mCircleDiameter / 2;
        if (DEBUG) Log.e("fish", "init+-+mOriginalOffsetBottom==" + mOriginalOffsetBottom);
    ...
    void setAnimationProgress(float progress) {
        if (isAlphaUsedForScale()) {
            ...
            if (DEBUG) Log.e(LOG_TAG, "setAnimationProgress,setColorViewAlpha");
        } else {
            //不出所料，我走的是这个方法
        ...
        if (mTarget == null) {//手势目标
            for (int i = 0; i < getChildCount(); i++) {
                ...
                if (!child.equals(mCircleView) && !child.equals(mCircleViewBottom)) {
                    mTarget = child;
                    if (DEBUG) Log.e(LOG_TAG, "ensureTarget,mTarget==" + mTarget.toString());
                    break;
                }
            }
        }
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        if (DEBUG) Log.e(LOG_TAG, "onLayout,childTop==" + childTop + ",childHeight" + childHeight);
       ...
       /* if(firstMeasure) {//其实已经没有必要摆在这里了
            ...
            mSpinnerBottomOffsetEnd = mOriginalOffsetBottom - mSpinnerOffsetEnd;
            firstMeasure = false;
            if (DEBUG) Log.e("fish", "firstMeasure,mCurrentTargetOffsetBottom==" + mCurrentTargetOffsetBottom + ",mSpinnerBottomOffsetEnd==" + mSpinnerBottomOffsetEnd);
            if (DEBUG) Log.e("fish", "firstMeasure,mOriginalOffsetBottom==" + mOriginalOffsetBottom);
        }*/
...
//                     (width / 2 + circleWidth / 2), childHeight/2 + circleHeight);
    ...
    public void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        for (int index = 0; index < getChildCount(); index++) {
            if (getChildAt(index) == mCircleView) {
                ...
                if (DEBUG) Log.e(LOG_TAG, "mCircleViewIndex==" + mCircleViewIndex);
                break;
            }
        }
        ...
        for (int index = 0; index < getChildCount(); index++) {
            if (getChildAt(index) == mCircleViewBottom) {
                ...
                if (DEBUG) Log.e(LOG_TAG, "mCircleViewBottomIndex==" + mCircleViewBottomIndex);
                break;
            }
        }
    ...
    public boolean canChildScrollUp() {
        if (mChildScrollUpCallback != null) {
            if (DEBUG) Log.e("fish", "canChildScrollUp;mChildScrollUpCallback != null");
            return mChildScrollUpCallback.canChildScrollUp(this, mTarget);

        }
        if (android.os.Build.VERSION.SDK_INT < 14) {
            if (DEBUG) Log.e("fish", "canChildScrollUp;Build.VERSION.SDK_INT < 14");
            if (mTarget instanceof AbsListView) {
                if (DEBUG) Log.e("fish", "canChildScrollUp;mTarget instanceof AbsListView");
            ...
            // if (DEBUG) Log.e("fish","canChildScrollUp;android.os.Build.VERSION.SDK_INT > 14");
            //这个可以用，判断是否可以向下拉
            // boolean re = ViewCompat.canScrollVertically(mTarget,1);
            //if (DEBUG) Log.e("fish","canChildScrollDown?"+re);
    ...
    /**
     * Set a callback to override {@link GZoomSwipeRefresh#canChildScrollUp()} method. Non-null
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        switch (action) {
            case MotionEvent.ACTION_DOWN:
                if (DEBUG) Log.e("fish", "onInterceptTouchEvent,ACTION_DOWN");
            ...
            case MotionEvent.ACTION_MOVE://这个move基本不触发
                if (DEBUG) Log.e("fish", "onInterceptTouchEvent,ACTION_MOVE");
            ...
            case MotionEvent.ACTION_UP:
                if (DEBUG) Log.e("fish", "onInterceptTouchEvent,motionEvent.ACTION_UP");
            case MotionEvent.ACTION_CANCEL:
                if (DEBUG) Log.e("fish", "onInterceptTouchEvent,MotionEvent.ACTION_CANCEL");
                mIsBeingDragged = false;
    ...
    public void onNestedScrollAccepted(View child, View target, int axes) {
        if (DEBUG) Log.e(LOG_TAG, "onNestedScrollAccepted,axes=" + axes);
    ...
    public void onNestedPreScroll(View target, int dx, int dy, int[] consumed) {
        ...
        if (DEBUG) Log.e(LOG_TAG, "onNestedPreScroll,dy=" + dy + ",consume[1]==" + consumed[1]);
    ...
    public void onStopNestedScroll(View target) {
        ...
        if (DEBUG) Log.e(LOG_TAG, "onStopNestedScroll,mTotalUnconsumed=" + mTotalUnconsumed);
        ...
        if (mTotalUnconsumedBottom > 0 && !mRefreshing) {
            if (DEBUG) Log.e("fish", "onStopNestedScroll,mTotalUnconsumedBottom > 0");
    ...
    public boolean dispatchNestedPreScroll(int dx, int dy, int[] consumed, int[] offsetInWindow) {
        if (DEBUG) Log.e("fish", "父：dispatchNestedPreScroll,dy=" + dy);
        //先拦截
    ...
    private void moveBottomSpinner(float overscrollTop) {
        ...
        if (!mScale) {
            ...
            if (DEBUG) Log.e("param", "mScale==false,in moveBottomSpinner");
        }
    ...
    private void moveSpinner(float overscrollTop) {
        if (DEBUG) Log.e("moveSpinner", "+++++++++++++++++++++++moveSpinner--" + overscrollTop);
        //overscrollTop = -overscrollTop;
        mProgress.showArrow(true);
        float originalDragPercent = overscrollTop / mTotalDragDistance;
        if (DEBUG) Log.e("moveSpinner", "moveSpinner--originalDragPercent==" + originalDragPercent);
        float dragPercent = Math.min(1f, Math.abs(originalDragPercent));
        if (DEBUG) Log.e("moveSpinner", "moveSpinner--dragPercent==" + dragPercent);
        float adjustedPercent = (float) Math.max(dragPercent - .4, 0) * 5 / 3;//这个不理解
        if (DEBUG) Log.e("moveSpinner", "moveSpinner--adjustedPercent==" + adjustedPercent);
        float extraOS = Math.abs(overscrollTop) - mTotalDragDistance;//这样不就是负的吗//就是负的//可以是正的
        if (DEBUG) Log.e("moveSpinner", "moveSpinner--extraOS==" + extraOS);
        float slingshotDist = mUsingCustomStart ? mSpinnerOffsetEnd - mOriginalOffsetTop
                : mSpinnerOffsetEnd;///mSpinnerOffsetEnd 默认是拉到最底的可能位置 ，和mTotalDragDistance一开始初始化是相同的
        if (DEBUG)
        ...
        if (DEBUG) Log.e("moveSpinner", "moveSpinner--tensionPercent==" + tensionPercent);
        float extraMove = (slingshotDist) * tensionPercent * 2;//我理解了，这个是模拟后来的拖动，如果
        if (DEBUG) Log.e("moveSpinner", "moveSpinner--extraMove==" + extraMove);
        int targetY = mOriginalOffsetTop + (int) ((slingshotDist * dragPercent) + extraMove);//最大可以达到 +2 slingshotDist
        if (DEBUG) Log.e("moveSpinner", "moveSpinner--targetY==" + targetY);
        ...
        //  if (DEBUG) Log.e(LOG_TAG,"mCurrentTargetOffsetTop=="+mCurrentTargetOffsetTop+",targetY=="+targetY);
    }

    private void finishSpinner(float overscrollTop) {
        if (DEBUG) Log.e("fish", "finishSpinner--" + overscrollTop);
        if (overscrollTop > mTotalDragDistance) {//大于这个核心点就升上去
            setRefreshing(true, true /* notify */);
            if (DEBUG) Log.e("fish", "finishSpinner-->>>overscrollTop > mTotalDragDistance");
        } else {
            // cancel refresh
            mRefreshing = false;
    ...
    private void finishSpinnerBottom(float overscrollTop) {//在小于mTotalDragDistance的时候处理的不好，会在中间停止
        if (DEBUG) Log.e("fish", "finishSpinnerBottom--" + overscrollTop);
        if (overscrollTop > mTotalDragDistance) {
            setRefreshingBottom(true, true);
            if (DEBUG) Log.e("fish", "finishSpinnerBottom-->>>overscrollTop > mTotalDragDistance");
        } else {
            // cancel refresh
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        ...
        if (DEBUG) Log.e("fish", "-onTouchEvent-;ACTION==" + ev.getAction());
        if (!isEnabled() || mReturningToStart || canChildScrollUp()
                || mRefreshing || mNestedScrollInProgress) {
            // Fail fast if we're not in a state where a swipe is possible
            return false;
        ...
        switch (action) {
            case MotionEvent.ACTION_DOWN:
                if (DEBUG) Log.e("fish", "-onTouchEvent-;ACTION_DOWN");
                mActivePointerId = ev.getPointerId(0);
                mIsBeingDragged = false;
                break;

            case MotionEvent.ACTION_MOVE: {
                if (DEBUG) Log.e("fish", "-onTouchEvent-;ACTION_MOVE");
                pointerIndex = ev.findPointerIndex(mActivePointerId);
                if (pointerIndex < 0) {
                    if (DEBUG)
            ...
            case MotionEventCompat.ACTION_POINTER_DOWN: {
                if (DEBUG) Log.e("fish", "-onTouchEvent-;MotionEventCompat.ACTION_POINTER_DOWN");
                pointerIndex = MotionEventCompat.getActionIndex(ev);
                if (pointerIndex < 0) {
                    if (DEBUG) Log.e(LOG_TAG,
            ...
            case MotionEventCompat.ACTION_POINTER_UP:
                if (DEBUG) Log.e("fish", "-onTouchEvent-;MotionEventCompat.ACTION_POINTER_UP");
                onSecondaryPointerUp(ev);
                break;

            case MotionEvent.ACTION_UP: {
                if (DEBUG) Log.e("fish", "-onTouchEvent-;MotionEvent.ACTION_UP");
                pointerIndex = ev.findPointerIndex(mActivePointerId);
                if (pointerIndex < 0) {
                    if (DEBUG)
    ...
    void setTargetOffsetTopAndBottom(int offset, boolean requiresUpdate) {
        if (DEBUG) Log.e(LOG_TAG, "setTargetOffsetTopAndBottom,offset==" + offset);
    ...
    /**
     * Classes that wish to override {@link GZoomSwipeRefresh#canChildScrollUp()} method
     * behavior should implement this interface.
     */
    public interface OnChildScrollUpCallback {
        /**
         * Callback that will be called when {@link GZoomSwipeRefresh#canChildScrollUp()} method
         ...
         */
        boolean canChildScrollUp(GZoomSwipeRefresh parent, @Nullable View child);
    }
}
